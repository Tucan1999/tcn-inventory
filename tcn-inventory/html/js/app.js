...
function showBlur() {
    $.post('https://tcn-inventory/showBlur');
}
...
//     localStorage.setItem('tcn-inventory-color', color);
// }

// const savedColor = localStorage.getItem('tcn-inventory-color');
...
function FormatAttachmentInfo(data) {
    $.post(
        "https://tcn-inventory/GetWeaponData",
        JSON.stringify({
            weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post(
                "https://tcn-inventory/RemoveAttachment",
                JSON.stringify({
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post(
            "https://tcn-inventory/combineWithAnim",
            JSON.stringify({
        ...
        $.post(
            "https://tcn-inventory/combineItem",
            JSON.stringify({
...
function optionSwitch(
    ...
    $.post(
        "https://tcn-inventory/SetInventoryData",
        JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            $.post("https://tcn-inventory/PlayDropSound", JSON.stringify({}));
            $.post(
                "https://tcn-inventory/SetInventoryData",
                JSON.stringify({
                    fromInventory: $fromInv.attr("data-inventory"),
            ...
            if (fromData.amount == $toAmount) {
                ...
                if (
                    ...
                    $.post(
                        "https://tcn-inventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://tcn-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $.post("https://tcn-inventory/CloseInventory", JSON.stringify({}));
