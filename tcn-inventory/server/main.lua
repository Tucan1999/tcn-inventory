...
local function CheckVersion()
    PerformHttpRequest('https://raw.githubusercontent.com/Rexshack-RedM/tcn-inventory/main/version.txt', function(err, text, headers)
...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
    ...
    if not ply.state.inv_busy then
        if name and id then
            ...
            TriggerClientEvent("tcn-inventory:client:closeinv", id)
...
RegisterNetEvent('tcn-inventory:server:SaveStashItems', function(stashId, items)
    MySQL.insert('INSERT INTO stashitems (stash, items) VALUES (:stash, :items) ON DUPLICATE KEY UPDATE items = :items', {
        ['stash'] = stashId,
        ['items'] = json.encode(items)
    })
    ...
    if amount <= item.amount then
        ...
        if RemoveItem(src, item.name, amount, item.slot) then
            if AddItem(target, item.name, amount, false, item.info, item.created) then
                ...
                TriggerClientEvent('tcn-inventory:client:giveAnim', src)
                TriggerClientEvent('tcn-inventory:client:giveAnim', target)
            else
...
RSGCore.Functions.CreateCallback('tcn-inventory:server:GetStashItems', function(_, cb, stashId)
